...
PS_CARD_WHITE_LOW  = np.array([0,   0, 200])
PS_CARD_WHITE_HIGH = np.array([180, 40, 255])
...
class CardDetector:
    ...
    def _find_card_rectangles(self, img):
        ...
        for cnt in contours:
            ...
            min_area = (w * h) * 0.003   # ≥ 0.3% da imagem
            max_area = (w * h) * 0.15    # ≤ 15% da imagem
    ...
    def _split_hole_board(self, rects, img_height):
        """
        No PokerStars Mobile:
        - Hole cards ficam na metade inferior (y > 55% da altura)
        - Board fica no centro (25% < y < 60%)
        """
        threshold = img_height * 0.55

        hole  = [r for r in rects if r[1] + r[3]*0.5 > threshold]
        board = [r for r in rects if r[1] + r[3]*0.5 <= threshold]

        # Se não separou bem, usa os 2 mais à direita/centro como hole
        if not hole and len(rects) >= 2:
            hole  = sorted(rects, key=lambda r: r[1], reverse=True)[:2]
            board = [r for r in rects if r not in hole]

        return sorted(hole, key=lambda r: r[0]), sorted(board, key=lambda r: r[0])
